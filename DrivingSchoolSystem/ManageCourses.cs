...
namespace DrivingSchoolSystem
{
    public partial class ManageCourses : Form
    {
        ...
        private void ManageCourses_Load(object sender, EventArgs e)
        {
            ...
            foreach (var course in Singleton.Courses)
            {
                dataGridView1.Rows.Add(
                    course.CourseId,
                    course.Type,
                    course.Fee.ToString("C")
                );
            }
        }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                int selectedId = Convert.ToInt32(selectedRow.Cells["Course ID"].Value);

                Course courseToRemove = Singleton.Courses.Find(course => course.CourseId == selectedId);
                if (courseToRemove != null)
                {
                    Singleton.Courses.Remove(courseToRemove);
                }
